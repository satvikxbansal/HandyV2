...
| T9 | Multi-step plan goes off-rails | Medium | User loses control of a recipe or unintended actions chain together | Deterministic recipes only; LLM chooses recipe and arguments but cannot emit free-form steps; max 6 steps; re-capture, re-validate, and audit each step; abort on app switch or failed verification | RecipeRunner tests reject over-6-step plans and runtime-injected steps; audit includes recipe, step index, policy decision, confirmation, and verification |
...
| T12 | Ride-hailing app UI drift breaks the recipe | Medium | Handy may fail to find the destination field/result or stop earlier than expected | Multi-matcher targets + graceful `Refused` + quarterly smoke | Ride-hailing recipe tests cover stable selectors; manual Uber smoke records screenshot path in `DEBUG_LOG.md` |
