...
function Section({ theme, icon, title, subtitle, children }) {
  return (
    <div style={{ padding: "26px 20px 4px" }}>
      {/* Section header — icon, title, and subtitle locked together as one unit */}
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: -0.2, lineHeight: 1.2 }}>{title}</div>
          {subtitle && (
            <div style={{ fontSize: 12.5, color: theme.textSecondary,
                          lineHeight: 1.4, marginTop: 3 }}>{subtitle}</div>
          )}
        </div>
      </div>
