...
object CapabilityDocs {
    ...
    fun generate(root: File) {
        generatedFiles(root).forEach { (file, text) ->
    ...
    fun staleFiles(root: File): List<String> =
        generatedFiles(root).filter { (file, expected) ->
            !file.exists() || file.readText() != expected
        }.keys.map { it.relativeTo(root).path }

    private fun generatedFiles(root: File): Map<File, String> {
        val capabilities = parse(root.resolve("docs/CAPABILITIES.yaml"))
    ...
    private fun parse(file: File): List<Capability> {
        ...
        file.readLines().forEachIndexed { index, rawLine ->
            ...
            val propMatch = Regex("^    ([A-Za-z0-9_]+):\\s*(.+)$").matchEntire(line)
