...
class PromptCatalogTest {
    ...
    @Test fun `quick overlay prompt uses recipe directives instead of raw executable plans`() {
        ...
        assertThat(prompt).contains("open_app, app_search, set_alarm")
        assertThat(prompt).contains("open spotify → [INTENT:open_app]")
        assertThat(prompt).contains("play jazz on spotify → [INTENT:app_search]")
        ...
        assertThat(prompt).contains("schedule dentist tomorrow 3 pm → [INTENT:create_calendar_event]")
        ...
        assertThat(prompt).contains("YouTube, YouTube Music, Spotify, or Chrome")
        assertThat(prompt).contains("open the latest podcast on youtube → [INTENT:youtube_search]")
