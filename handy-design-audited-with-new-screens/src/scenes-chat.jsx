...
function ContextBar({
  ...
  iconTone = "point",
  icon = "eye",
  editing = false,   // pill-only: turns the label into an inline text field
  draftValue,        // value shown in the inline field
}) {
  const t = useTheme();
  ...
  if (variant === "pill") {
    // The pill now ships with a paired minimise affordance — a 36 dp
    // circular sibling that sits just to the right of the pill, sharing
    // the same blur/border vocabulary. Together they read as a single
    // floating cluster above the composer.
    //
    // Editing mode (`editing=true`) expands the pill: the label becomes
    // an inline text field with an amber accent caret + Cancel/Save
    // affordances. Width grows to fit; minimise button stays in place.
    return (
      <div style={{
        display: "flex", alignItems: "center", gap: 8,
        alignSelf: "center", maxWidth: "100%",
      }}>
        {/* Context pill — bears the editing state */}
        <div style={{
          display: "inline-flex", alignItems: "center", gap: 10,
          padding: editing ? "6px 6px 6px 8px" : "8px 14px 8px 8px",
          borderRadius: 999,
          background: "rgba(24,26,31,0.78)",
          border: `0.5px solid ${editing ? t.colors.pointHair : "rgba(255,255,255,0.12)"}`,
          ...
          {editing ? (
            <>
              {/* Inline editable text with blue caret — no chip wrapper */}
              <span style={{
                ...
                <span style={{
                  ...
                  width: 1.5, height: 14, marginLeft: 1,
                  background: t.colors.point,
                  animation: "handy-caret-blink 1s steps(2, start) infinite",
                }} />
              </span>

              {/* Action cluster — Cancel (text) + Save (blue pill) */}
              <span style={{
                font: `500 11px/1 ${HANDY_TYPE.fontBody}`,
                color: t.colors.textMuted, padding: "0 6px",
                cursor: "pointer",
              }}>Cancel</span>
              <button style={{
                padding: "6px 12px",
                ...
                background: t.colors.point, color: "#FFFFFF",
            ...
            <>
              ...
              <span style={{
                font: `500 11px/1 ${HANDY_TYPE.fontBody}`,
                color: accent, cursor: "pointer", flex: "0 0 auto",
              }}>Change</span>
            </>
          )}
        </div>

        {/* Sibling minimise button — same surface, same hairline */}
        <button style={{
          ...
          <Illu name="minimise" size={14} color={t.colors.textSecondary} />
        </button>
      </div>
    );
  }
...
// ────────────────────────────────────────────────────────────────────────
function ChatActive({ contextBarPosition = "top", app = "Google Maps", editing = false } = {}) {
  const t = HANDY_TOKENS.amber;
      ...
      <Phone>
        <div style={{
          ...
          <FloatingComposer
            ...
            bottomChrome={showBottom ? <ContextBar app={app} variant="pill" editing={editing} draftValue={editing ? "Maps · Valencia run" : null} /> : null}
          />
        </div>
      </Phone>
    </ThemeProvider>
