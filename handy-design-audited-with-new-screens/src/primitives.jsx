...
// ───── Toggle switch ──────────────────────────────────────────────────────
// Compose: HandySwitch(checked: Boolean, onCheckedChange, tone)
// `tone` lets a section override the on-state colour to match its accent
// (e.g. the Voice section uses honey instead of amber).
function Toggle({ on = false, disabled = false, tone }) {
  const t = useTheme();
...
function Composer({ placeholder = "Ask Handy anything…", glass = false }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${glass ? "rgba(255,255,255,0.16)" : t.colors.borderSubtle}`,
      backdropFilter: glass ? "blur(24px)" : "none",
    }}>
      <button style={{
        width: 44, height: 44, borderRadius: "50%",
        background: glass ? "rgba(255,255,255,0.08)" : t.colors.accentSoft,
        border: glass ? "1px solid rgba(255,255,255,0.10)" : "none",
        cursor: "pointer",
        display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 auto",
      }}>
        <Illu name="mic" size={20} color={glass ? t.colors.textPrimary : t.colors.accent} />
      </button>
      <div style={{
        flex: 1, minWidth: 0,
        color: t.colors.textMuted,
      ...
      <button style={{
        width: 44, height: 44, borderRadius: "50%",
        background: glass ? "rgba(255,255,255,0.08)" : t.colors.accent,
        border: glass ? "1px solid rgba(255,255,255,0.10)" : "none",
        cursor: "pointer",
        display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 auto",
      }}>
        <Illu name="send" size={18} color={glass ? t.colors.accent : t.colors.accentInk} />
      </button>
    </div>
  );
}
